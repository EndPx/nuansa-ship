#!/bin/bash
# Re-copy sanitized sources + rebuild using vendored stdlib.
set -e
export PATH=/home/albary/go/bin:/home/albary/.local/bin:/usr/local/bin:/usr/bin:/bin

SRC="/mnt/d/Belajar/Hackacton/NuansaShip Initiate/contracts/sources"
DST="$HOME/nuansa-ship-contracts"

echo "=== RE-SYNC SOURCES ==="
rm -rf "$DST/sources"
mkdir -p "$DST/sources"
cp "$SRC"/*.move "$DST/sources/"
ls "$DST/sources/"
...
cd "$DST"
rm -rf build
timeout 240 minitiad move build --named-addresses "nuansa_ship=$HEX_PADDED" --path . 2>&1 | tail -50
